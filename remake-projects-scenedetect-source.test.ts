...
const prismaMock = vi.hoisted(() => ({
  ...
  remakeSource: {
    findFirst: vi.fn(async (): Promise<{ id: string; sourceRevision: number; operationKey: string; probeMetadata: string } | null> => null),
...
const probeMock = vi.hoisted(() => ({
  probeVideo: vi.fn(async () => ({ duration: 2, fps: 30, width: 1920, height: 1080, totalFrames: 60 })),
}))
...
function sourceRequest(file: File | null, operationKey = 'upload-1') {
...
describe('POST /api/remake-projects/[projectId]/source', () => {
  ...
  it('persists a server-probed source revision and replays the same operation without another upload', async () => {
    ...
    prismaMock.remakeSource.findFirst.mockResolvedValueOnce({ id: 'source-1', sourceRevision: 1, operationKey: 'upload-1', probeMetadata: JSON.stringify({ duration: 2, totalFrames: 60 }) })
