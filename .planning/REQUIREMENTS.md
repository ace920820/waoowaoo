...
- [ ] **RMP-02**: 用户可以在翻拍项目中查看原始视频、Shot 总数、当前阶段、完成度、失败项和待审核项
...
- [ ] **SHOT-02**: 用户可以从 waoowaoo 发起 SceneDetect 镜头切分，不需要切换到独立工具手工搬运结果
- [ ] **SHOT-03**: 系统可以接收并持久化 SceneDetect 输出的 Shot 序号、起止时间、时长、原始片段和候选关键帧
- [ ] **SHOT-04**: 用户可以在同一审核界面通过原视频播放器、时间轴和 Shot 列表检查切分结果
- [ ] **SHOT-05**: 用户可以调整 Shot 起止边界、删除误切 Shot、拆分/合并相邻 Shot，并重新生成受影响的原始片段
- [ ] **SHOT-06**: 用户可以为每个 Shot 从原始视频中选择或重新提取 Start / Middle / End 关键帧
...
- [ ] **TASK-02**: Task 通过 `task_type + executor + capability` 路由到 SceneDetect、Codex/LLM、图片 Worker 或视频 Worker，业务页面不直接控制 CLI/进程
...
| 重写 SceneDetect 镜头识别算法 | 现有能力可复用，本轮只实现稳定集成和审核体验 |
