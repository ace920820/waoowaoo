...
- [x] `08-01-PLAN.md` — 审计并最小扩展现有 Remake 输出/溯源/失效 schema，持久化选择、不可变批次/候选/采用历史与 revision 动作表
- [x] `08-02-PLAN.md` — 复用现有 Task、图片队列/Worker、模型网关、存储和配置，完成 Remake 生成/API/采用/动作表/失效后端闭环
- [x] `08-03-PLAN.md` — 以薄 Remake 适配器接入原版分镜/成片与完整共享工具，完成双层 Shot、批次比较/采用和禁用视频提交
- [x] `08-04-PLAN.md` — 聚焦验证刷新持久化、动作表失效、共享工具、桌面/移动端与零视频提交/Task，并完成人工验收（E2E 10/10 通过，人工验收待执行）
- [x] `08-05-PLAN.md` — 2×3 分镜工作台布局、原始帧点击选择、逐槽 Prompt 展示与联动
- [x] `08-06-PLAN.md` — 新画面大图预览、候选切换、下载、重新生成、采用与恢复上一版本
- [x] `08-07-PLAN.md` — 场景/角色/物品资产选择器，复用经典分镜页的资产系统与交互模式
- [x] `08-08-PLAN.md` — 画面描述 Prompt 编辑与自动采用 + 经典新画面卡片交互，E2E 验收修复
...
- [ ] `09-01-PLAN.md` — 接通单 Shot 精确输入、不可变快照与现有视频生成执行链路
- [ ] `09-02-PLAN.md` — 完成版本备注、显式采用历史、上游失效与重新确认闭环
- [ ] `09-03-PLAN.md` — 启用手动多图选择、能力参数、播放历史与采用的成片页产品 UI
- [ ] `09-04-PLAN.md` — 完成迁移、单元/API/Worker、Playwright、真实生成与人工验收验证
