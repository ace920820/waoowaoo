...
### 已落地的最小能力（2026-04-14 补记）
已在 `stage=videos` 的单 panel 卡片内落地一个 **轻量 dialogue override**：
- 用户可直接修改当前 panel 下次视频生成要说的台词
- 覆盖只作用于当前 panel 的视频生成，不回写 screenplay
- 实际执行优先级为：`dialogueOverride > matched voice lines > panel text fallback`
- 该能力刻意不扩成完整剧本编辑器 / 多 panel 批量对白配置面板
...
### 原则
先做“可观测”，后做“可编辑”；即使进入可编辑，也先保持 panel 级最小纠偏，不把界面一次性做成重型对白编辑器。
