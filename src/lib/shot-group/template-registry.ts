...
export type ShotGroupTemplateSpec = {
  ...
  assetConstraintInstruction: string
  slotTitles: string[]
}
...
export const SHOT_GROUP_TEMPLATE_REGISTRY: Record<NovelPromotionShotGroupTemplateKey, ShotGroupTemplateSpec> = {
  'grid-4': {
    ...
    assetConstraintInstruction: '优先遵守手动资产选择，其次遵守系统预选；缺失约束时允许按剧本文本自然回退。',
...
export function getShotGroupTemplateSpec(templateKey: string | null | undefined): ShotGroupTemplateSpec {
  const normalized = (templateKey || 'grid-9') as NovelPromotionShotGroupTemplateKey
  return SHOT_GROUP_TEMPLATE_REGISTRY[normalized] || SHOT_GROUP_TEMPLATE_REGISTRY['grid-9']
}
