...
export async function handleShotGroupImageTask(job: Job<TaskJobData>) {
  ...
  const modelKey = targetField === 'reference'
    ? (projectModels.shotGroupReferenceImageModel || projectModels.storyboardModel)
    : projectModels.storyboardModel
  if (!modelKey) {
    throw new Error(targetField === 'reference'
      ? 'Shot group reference image model not configured'
