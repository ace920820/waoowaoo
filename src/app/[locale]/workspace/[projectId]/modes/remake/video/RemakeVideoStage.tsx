...
function MediaCard({ projectId, mediaId, label }: { projectId: string; mediaId: string; label: string }) { return <div className="overflow-hidden rounded border border-slate-200"><img src={mediaUrl(projectId, mediaId) ?? ''} alt={label} className="aspect-video w-full object-cover" /><p className="p-2 text-xs text-slate-600">{label}</p></div> }
