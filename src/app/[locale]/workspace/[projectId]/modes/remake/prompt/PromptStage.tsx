...
export function PromptStage({ projectId, snapshot }: Props) {
  ...
  return <section className="prompt-stage" data-testid="remake-prompt-stage">
    <header className="prompt-header">
      ...
      <div className="prompt-header-actions"><span className="prompt-queue"><Clock3 size={15} /> {videoTask ? t('running') : `${snapshot.tasks.filter((task) => task.status === 'queued').length} ${t('queued')}`}</span><button type="button" className="prompt-primary" disabled={analyze.isPending || eligible === 0} onClick={analyzeVideo}><Play size={15}/>{hasVideo ? t('reanalyzeVideo') : t('analyzeVideo')}</button></div>
    </header>
...
function FrameCard({ slot, shot, projectId, expanded, onToggle }: { slot: Slot; shot: RemakeSnapshot['shots'][number]; projectId: string; expanded: boolean; onToggle: () => void }) {
  ...
  return <article className="prompt-frame"><header><b>{slot.toUpperCase()}</b><span>{slot === 'start' ? '起始帧' : slot === 'middle' ? '中间帧' : '结束帧'}</span><em className={`prompt-state ${current}`}>{stateLabel(current, t)}</em></header>{frame?.mediaUrl ? <a href={frame.mediaUrl} target="_blank" rel="noreferrer"><img src={frame.mediaUrl} alt={`${slot} frame`} /></a> : <div className="prompt-media-empty">无关键帧</div>}
    {!track?.latestVersion ? <button className="prompt-primary" disabled={!shot.review?.promptEligible || analyze.isPending} onClick={submit}><Play size={14}/>{current === 'failed' ? t('retry') : '分析图片 Prompt'}</button> : <><div className="prompt-version"><span>{t('latest')} v{track.latestVersion.versionNumber}</span>{track.adoptedVersion ? <small><Check size={13}/>{t('adopted')} v{track.adoptedVersion.versionNumber}</small> : null}</div><button className="prompt-detail-toggle" onClick={onToggle}>{t('fullAnalysis')}{expanded ? <ChevronUp size={15}/> : <ChevronDown size={15}/>}</button>{expanded ? <p className="prompt-detail-note">完整结果、原始输出和版本比较可从版本记录中查看。</p> : null}{track.latestVersion.reviewStatus === 'PENDING' ? <button className="prompt-secondary" disabled={approve.isPending} onClick={() => approve.mutate(track.latestVersion!.id)}><Check size={14}/>{t('approveAndAdopt')}</button> : null}<span className="prompt-history"><History size={14}/>{t('versionHistory')}</span></>}</article>
}

function VideoPanel({ shot, videoTask }: { shot: RemakeSnapshot['shots'][number]; videoTask?: string }) { const t = useTranslations('remakeWorkbench'); const track = trackFor(shot.promptTracks, 'video'); const current = state(track, videoTask); return <article className="prompt-video"><header><div><Sparkles size={18}/><h3>{t('videoPrompt')}</h3></div><em className={`prompt-state ${current}`}>{stateLabel(current, t)}</em></header><p>{track?.latestVersion ? '当前镜头的核心动作、调度与镜头运动结果可在版本详情中查看和编辑。' : '请使用页面顶部的“分析整段视频”来一次性分析全部已确认镜头。'}</p>{track?.adoptedVersion ? <small><Check size={14}/>{t('adopted')} v{track.adoptedVersion.versionNumber}</small> : null}<span className="prompt-history"><History size={14}/>{t('versionHistory')}</span></article> }
