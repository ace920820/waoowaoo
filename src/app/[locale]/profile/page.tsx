...
import StoryboardModeConfigTab from './components/StoryboardModeConfigTab'
...
export default function ProfilePage() {
  ...
  // 主要分区：扣费记录 / API配置
  const [activeSection, setActiveSection] = useState<'billing' | 'apiConfig' | 'storyboardModes'>('apiConfig')
          ...
          <div className="w-64 flex-shrink-0">
            <div className="glass-surface-elevated h-full flex flex-col p-5">
              ...
              <nav className="flex-1 space-y-2">
                ...
                <button
                  onClick={() => setActiveSection('storyboardModes')}
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left transition-all cursor-pointer ${activeSection === 'storyboardModes'
                    ? 'glass-btn-base glass-btn-tone-info'
                    : 'text-[var(--glass-text-secondary)] hover:bg-[var(--glass-bg-muted)]'
                    }`}
                >
                  <AppIcon name="film" className="w-5 h-5" />
                  <span className="font-medium">分镜模式</span>
                </button>
              </nav>
              {/* 退出登录 */}
              <button
          ...
          <div className="flex-1 min-w-0">
            <div className="glass-surface-elevated h-full flex flex-col">
              ...
              {activeSection === 'apiConfig' ? (
                <ApiConfigTab />
              ) : activeSection === 'storyboardModes' ? (
                <StoryboardModeConfigTab />
              ) : (
