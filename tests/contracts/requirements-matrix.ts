...
export const REQUIREMENTS_MATRIX: ReadonlyArray<RequirementCoverageEntry> = [
  {
    id: 'IPRM-01',
    feature: 'Manual Start/Middle/End image Prompt analysis',
    userValue: 'Each confirmed keyframe can be analyzed independently without a browser-to-CLI path.',
    risk: 'slot actions drift from the Task/Worker contract or become bulk actions',
    priority: 'P0',
    tests: ['tests/unit/remake-projects/prompt-contract.test.ts', 'tests/integration/task/remake-prompt-image.test.ts', 'tests/e2e/remake-prompt.spec.ts'],
  },
  {
    id: 'IPRM-02',
    feature: 'Structured image Prompt result',
    userValue: 'Users retain complete analysis plus the integrated Prompt and negative constraints.',
    risk: 'partial parsing drops stable display fields or raw evidence',
    priority: 'P0',
    tests: ['tests/unit/remake-projects/prompt-contract.test.ts', 'tests/integration/task/remake-prompt-image.test.ts', 'tests/fixtures/remake-prompt/codex-events.json'],
  },
  {
    id: 'IPRM-03',
    feature: 'Image Prompt provenance',
    userValue: 'A result is traceable to the Task, Skill, schema, model, and executor.',
    risk: 'untraceable output cannot be audited or rerun safely',
    priority: 'P0',
    tests: ['tests/integration/task/remake-prompt-image.test.ts'],
  },
  {
    id: 'IPRM-04',
    feature: 'Editable image Prompt versions',
    userValue: 'A user can inspect, edit, and retain Prompt history for a keyframe.',
    risk: 'an edit overwrites immutable analysis history',
    priority: 'P0',
    tests: ['tests/integration/api/remake-projects-prompt-review.test.ts', 'tests/e2e/remake-prompt.spec.ts'],
  },
  {
    id: 'IPRM-05',
    feature: 'Image Prompt rerun and adoption history',
    userValue: 'A pending rerun never silently replaces the adopted Prompt.',
    risk: 'latest and adopted pointers are conflated',
    priority: 'P0',
    tests: ['tests/integration/api/remake-projects-prompt-review.test.ts', 'tests/e2e/remake-prompt.spec.ts'],
  },
  {
    id: 'IPRM-06',
    feature: 'Explicit image Prompt approval',
    userValue: 'Only an approved, adopted version becomes a downstream default.',
    risk: 'Task completion is confused with human approval',
    priority: 'P0',
    tests: ['tests/integration/api/remake-projects-prompt-review.test.ts'],
  },
  {
    id: 'IPRM-07',
    feature: 'Independent image failure and retry',
    userValue: 'One failed keyframe does not block the remaining image Prompt work.',
    risk: 'shared failure state corrupts sibling tasks',
    priority: 'P0',
    tests: ['tests/integration/task/remake-prompt-concurrency.test.ts', 'tests/e2e/remake-prompt.spec.ts'],
  },
  {
    id: 'VPRM-01',
    feature: 'Whole-video Prompt input',
    userValue: 'One request analyzes the source video with every confirmed Shot and keyframe.',
    risk: 'per-Shot execution produces inconsistent results',
    priority: 'P0',
    tests: ['tests/unit/worker/remake-prompt.test.ts', 'tests/integration/task/remake-prompt-video-atomic.test.ts'],
  },
  {
    id: 'VPRM-02',
    feature: 'Structured video Prompt result',
    userValue: 'Each Shot has an editable core event with recoverable detailed analysis.',
    risk: 'invalid model output reaches persistent Prompt versions',
    priority: 'P0',
    tests: ['tests/unit/worker/remake-prompt.test.ts', 'tests/integration/task/remake-prompt-video-atomic.test.ts'],
  },
  {
    id: 'VPRM-03',
    feature: 'Video Prompt provenance',
    userValue: 'Whole-run results retain source and executor provenance.',
    risk: 'result cannot be attributed to its input run',
    priority: 'P0',
    tests: ['tests/integration/task/remake-prompt-video-atomic.test.ts'],
  },
  {
    id: 'VPRM-04',
    feature: 'Video Prompt version review',
    userValue: 'Users can inspect a Shot result and append an edit without overwriting history.',
    risk: 'review UI makes a project-level run look like per-Shot generation',
    priority: 'P0',
    tests: ['tests/integration/api/remake-projects-prompt-review.test.ts', 'tests/e2e/remake-prompt.spec.ts'],
  },
  {
    id: 'VPRM-05',
    feature: 'Atomic video rerun history',
    userValue: 'All new versions appear together and old adoption survives pending reruns.',
    risk: 'partial writes and accidental adoption changes',
    priority: 'P0',
    tests: ['tests/integration/task/remake-prompt-video-atomic.test.ts', 'tests/e2e/remake-prompt.spec.ts'],
  },
  {
    id: 'VPRM-06',
    feature: 'Explicit video Prompt approval',
    userValue: 'Each Shot adoption remains independent after one whole-video run.',
    risk: 'approval of one track changes unrelated Prompt tracks',
    priority: 'P0',
    tests: ['tests/integration/api/remake-projects-prompt-review.test.ts', 'tests/e2e/remake-prompt.spec.ts'],
  },
  {
    id: 'VPRM-07',
    feature: 'Whole-run video retry',
    userValue: 'A failed video run preserves prior versions and retries as one run.',
    risk: 'retry creates a partial or per-Shot result set',
    priority: 'P0',
    tests: ['tests/integration/task/remake-prompt-video-atomic.test.ts', 'tests/e2e/remake-prompt.spec.ts'],
  },
  {
