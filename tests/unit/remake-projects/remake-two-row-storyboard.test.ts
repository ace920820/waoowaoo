...
describe('Remake 2×3 storyboard layout contract', () => {
  ...
  it('embeds the unit lifecycle panel with a management toggle (D-19 revised)', () => {
    ...
    // Shot overview badges unit membership (D-18 / Phase 09.2: #N + tone)
    expect(source).toContain('buildShotUnitBadgeMap')
    expect(source).toContain('unitBadges={shotUnitBadges}')
    expect(source).toContain('onJumpToUnit={jumpToUnit}')
  })
})
